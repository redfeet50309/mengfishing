...
function getFishByEnvironmentAndTime(environment, time) {
    // Roo: Updated filter logic to handle arrays
    return fishDatabase.filter(fish =>
        Array.isArray(fish.environments) && fish.environments.includes(environment) &&
        Array.isArray(fish.times) && fish.times.includes(time)
    );
}
